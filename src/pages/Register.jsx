...
function Register() {
  ...
  const handle = async (e) => {
    ...
    setSubmitting(true);
    try {
      await register({ name, email, password }); // saves the token
  ...
  return (
    <div className="mx-auto max-w-sm py-10">
      <div className="mb-6 flex flex-col items-center text-center">
        ...
        <p className="mt-1 text-sm text-neutral-500">Join a team, play matches, win tournaments.</p>
      </div>
      ...
      >
        <InputField label="Name" icon={User} value={name} onChange={setName} placeholder="Your display name" required />
